...
type player struct {
	...
	history      *deque.Deque[string] // todo: set maximum
...
func (p *player) Forward(forwardCount uint) error {
	p.forward <- forwardCount
	return nil
}
...
func (p *player) asyncPlayerStateControlRoutine() {
	for {
		var err error
		// TODO combine channels for correct queueing of commands
		select {
		case ctx := <-p.play:
		...
		//case <-p.idle:
		case fwc := <-p.forward:
			err = p.states.getState().Forward(fwc)
...
func (p *player) RemoveQueueFront(removeCount uint) {
	log.Printf("removing %d media from front of queue: ", removeCount)

	p.queueMutex.Lock()
	for i := uint(0); i < removeCount; i++ {
		if p.queue.Len() == 0 {
			break
		}
